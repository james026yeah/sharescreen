...
public class AirplayProcess extends HandlerThread {
    ...
    public void startAirplay() {
    	if(null != mHandler) {
    		mHandler.sendEmptyMessage(MSG_START_AIRPLAY);
    	}
    }
    ...
    public void startBonjour(){
        Thread thread = new Thread() {
     	public void run() {
     		...
     		boolean ok = mBAR.registerBonjourService();
     		if (ok) {
     			Log.d(TAG, "register Bonjour service ok!");
     		} else {
     			Log.d(TAG, "register Bonjour service error!");
     		}
     	}
     }; 
     thread.start();
    ...
    {
    	byte[] cacheBytes = new byte[image.length];
    	//System.arraycopy(image, 0, cacheBytes, 0, image.length);
