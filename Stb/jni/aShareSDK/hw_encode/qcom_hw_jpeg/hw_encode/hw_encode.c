...
void encoder_output_handler (void *p_user_data,
			     void *p_arg, jpeg_buffer_t buffer);
...
//	encoder_args.main.format = YCRCBLP_H2V2;
...
//    	if (img_info.color_format <= YCBCRLP_H1V1)
	...
	for (i = 0; i < p_args->back_to_back_count; i++)
	{
		...
		dest.buffer_cnt = 2;
		if (JPEG_FAILED (jpeg_buffer_init (&dest.buffers[0])) ||
		    JPEG_FAILED (jpeg_buffer_init (&dest.buffers[1])) ||
		    JPEG_FAILED (jpeg_buffer_allocate
				 (dest.buffers[0], 8192, use_pmem))
		    ||
		    JPEG_FAILED (jpeg_buffer_allocate
				 (dest.buffers[1], 8192, use_pmem)))
		{
			LOG_D ("hw_engine_encode: failed to allocate destination buffers\n");
			jpeg_buffer_destroy (&dest.buffers[0]);
			jpeg_buffer_destroy (&dest.buffers[1]);
			dest.buffer_cnt = 0;
		}
...
void encoder_output_handler (void *p_user_data, void *p_arg, jpeg_buffer_t buffer)
{
...
int hw_encode (struct hw_enc_param *param)
{
	...
	if(2 == param->bytes_per_pixel)
		rgb2yuv16bit (in_buf, g_y, g_uv, param->width, param->height);
	else if(4 == param->bytes_per_pixel)
