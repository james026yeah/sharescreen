...
struct AudioOutput
{
  ...
  void (*ao_set_progress)(const char *buffer, unsigned int size);
  void (*ao_set_play_status)(int state);
  void (*ao_set_volume)(float vol);
  void (*ao_set_device_tab)(const char *deviceId, int length);
}; 
