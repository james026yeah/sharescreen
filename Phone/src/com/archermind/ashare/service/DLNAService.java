...
import com.archermind.ashare.dlna.ServerProcess;
...
public class DLNAService extends Service {
	...
	private AshareProcess mAshareProc;
        ...
        public void handleMessage(Message msg) {
            ...
            case MessageDefs.MSG_MDMC_ON_PROC_PREPARED:
            	...
            	if(isWifiConnected() || isWifiApEnabled()) {
            		...
            		mMDMCProc.startMediaController();
            		mMDMCProc.searchDevices();
            	}
				break;
	...
	public void onDestroy() {
		...
		if(mMDMCProc != null) {
			mMDMCProc.quit();
			mMDMCProc = null;
			Log.v(TAG, "onDestroy() stop M-DMC process");
		}
		if(mMDMSProc != null) {
			mMDMSProc.quit();
			mMDMSProc = null;
	...
	private void startDevices() {
		Log.v(TAG, "------------------------start startDevices()");
		if(mMDMCProc != null && mMDMCProc.isAlive()) {
			Log.v(TAG, "------------------------start DMC()");
			mMDMCProc.startMediaController();
		}
		if(mMDMSProc != null && mMDMSProc.isAlive()) {
			Log.v(TAG, "------------------------start DMS()");
			mMDMSProc.startMediaServer();
		}
	}
	...
	private void stopDevices() {
		...
		if(mMDMCProc != null && mMDMCProc.isAlive()) {
			Log.v(TAG, "------------------------stop DMC");
			mMDMCProc.stopMediaController();
		}
		
		if(mMDMSProc != null && mMDMSProc.isAlive()) {
			Log.v(TAG, "------------------------stop DMS");
			mMDMSProc.stopMediaServer();
		}
	}
