...
#define LIBPATH "/data/data/archermind.dlna.mobile/lib/"
void *libsyslib = NULL;
int (*lib_ASurface_init)(JNIEnv* env, jobject jsurface, int sdkVersion, ASurface** aSurface);
void (*lib_ASurface_deinit)(ASurface** aSurface);
int (*lib_ASurface_lock)(ASurface* aSurface, AndroidSurfaceInfo* info);
void (*lib_ASurface_scaleToFullScreen_skia)(ASurface* aSurface, AndroidSurfaceInfo* src, AndroidSurfaceInfo *dst, size_t size);
int (*lib_ASurface_unlockAndPost)(ASurface* aSurface);

void init_native_surface(char *version)
{

    LOGD("%s wen++++++++++\n", __func__);
	char path[256];
	sprintf(path, "/data/data/archermind.dlna.mobile/lib/libishare-sys%s.so", version);
	libsyslib = dlopen(path, RTLD_NOW);
	if(!libsyslib)
	{
	...
	if(dlerror() != NULL)
	{
		LOGD("dlsym %s function error in %d line!\n", __func__, __LINE__);
	}
}

void deinit_native_surface(void)
{
    LOGD("dlsym %s function come in %d line!\n", __func__, __LINE__);
	if(libsyslib)
	{
		dlclose(libsyslib);
		libsyslib = NULL;
...
int surface_start(JNIEnv *env, jobject jsurface)
{
    ...
    LOGD("ro.build.version.sdk %s", value);

	LOGD("%s start\n", __func__);
	first_frame = 1;

	ret = lib_ASurface_init(env, jsurface, 15, &aSurface);
