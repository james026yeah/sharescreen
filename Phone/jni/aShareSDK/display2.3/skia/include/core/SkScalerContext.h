...
public:
    ...
    // add by mediatek start
#ifdef MTK_SHAPE_ENGINE_SUPPORT
    SkScalerContext* charToGlyphIndex(SkUnichar uni, uint16_t* index);
    uint16_t charToGlyphIndex(SkUnichar uni);
    unsigned getBaseGlyphCount() {
        return fBaseGlyphCount;
    }
    virtual bool coverScript(uint32_t table, uint32_t script){
        return false;
    }
    void* getSEFont() {
        return fShapeFont;
    }
    virtual uint32_t opentypeShaper(SETag script, SETag lang, const SEUnicode* input, int offset, int count, int extraOffset, const unsigned int* property, const int* offsetArray, const opentype_feature_item_struct* gsubFeature, const int gsubFeatureNum, const opentype_feature_item_struct* gposFeature, const int gposFeatureNum, GlyphString* out);
#endif /* MTK_SHAPE_ENGINE_SUPPORT */
    // add by mediatek end
