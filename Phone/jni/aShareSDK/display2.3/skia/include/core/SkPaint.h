...
#ifdef MTK_SHAPE_ENGINE_SUPPORT
class SkScalerContext;
#endif /* MTK_SHAPE_ENGINE_SUPPORT */
// add by mediatek end
...
// add by mediatek start
#ifdef MTK_SHAPE_ENGINE_SUPPORT
typedef SkScalerContext* (*SkGlyphCacheProc)(SkGlyphCache*, const char**, uint16_t*);
typedef const SkGlyph& (*SkMeasureCacheProcGlyph)(SkGlyphCache* cache, uint16_t gIndex, SkScalerContext* sContext);
typedef SkScalerContext* (*SkScalerCntxProc)(SkGlyphCache*, const char**, uint16_t*);
typedef const SkGlyph& (*SkGlyphDrawCacheProc)(SkGlyphCache*, unsigned int, SkFixed, SkFixed, SkScalerContext*);
typedef uint16_t (*SkGetChar)(const char**);
#endif /* MTK_SHAPE_ENGINE_SUPPORT */
// add by mediatek end
...
public:
    ...
    */
    ...
    /* add by mediatek to process text to glyphs with complex language process */
    int textToGlyphs(const void* textData, size_t byteLength,
                              uint16_t glyphs[], size_t glyphsLength) const;
    ...
    */
    ...
    // add by mediatek start
    SkScalar    measureComplexText(const void* text, size_t length,
                            SkRect* bounds, SkScalar scale = 0) const;
    SkScalar measureComplexText(const void* text, size_t length) const;
    // add by mediatek end
...
// add by mediatek start
    int getComplexTextWidths(const void* textData, size_t byteLength, SkScalar widths[], int clusterIndex[], SkRect bounds[] = NULL) const;
// add by mediatek end
...
// add by mediatek start
#ifdef MTK_SHAPE_ENGINE_SUPPORT
    SkGlyphCacheProc getGlyphCacheProc(TextBufferDirection tbd, bool needFullMetrics) const;
    SkScalerCntxProc getScalerCntxProc() const;
    SkGlyphDrawCacheProc getGlyphDrawCacheProc() const;
    SkGetChar getChar(TextBufferDirection tbd) const;
    void langCheck(const char* text, size_t byteLength, bool& isBidi, bool& isShape, SkUnichar* out = NULL, int* charLength = NULL) const;
#endif /* MTK_SHAPE_ENGINE_SUPPORT */
    SkScalar measure_complex_text(SkGlyphCache*, const char* text, size_t length,
